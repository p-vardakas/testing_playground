  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" data-test-id="checkout-container">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <h2 className="text-2xl font-semibold text-white mb-6" data-test-id="shipping-title">Shipping Information</h2>
          <form onSubmit={handleSubmit} className="space-y-4" data-test-id="checkout-form">
            <div>
              <label className="block text-sm font-medium text-white" data-test-id="name-label">Name</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-sm font-medium text-white" data-test-id="phone-label">Phone</label>
              <input
                type="tel"
                required
                className="input-primary"
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="col-span-2">
                <label className="block text-sm font-medium text-white" data-test-id="address-label">Address</label>
                <input
                  type="text"
                  required
                  className="input-primary"
              ...
              <div>
                <label className="block text-sm font-medium text-white" data-test-id="zip-label">ZIP Code</label>
                <input
                  type="text"
                  required
                  className="input-primary"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-white" data-test-id="city-label">City</label>
                <input
                  type="text"
                  required
                  className="input-primary"
              ...
              <div>
                <label className="block text-sm font-medium text-white" data-test-id="state-label">State</label>
                <input
                  type="text"
                  required
                  className="input-primary"
        ...
        <div>
          <h2 className="text-2xl font-semibold text-white mb-6" data-test-id="order-summary-title">Order Summary</h2>
          <div className="bg-white/5 backdrop-blur-sm rounded-lg p-6" data-test-id="order-summary-container">
            <div className="space-y-4 mb-6" data-test-id="cart-items-list">
              {cart.map((item) => (
                <div key={item.id} className="flex items-center space-x-4" data-test-id={`cart-item-${item.id}`}>
                  <div className="flex-1">
                    <p className="font-medium text-white" data-test-id={`item-name-${item.id}`}>{item.name}</p>
      ...
      {showConfirmation && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50" data-test-id="order-confirmation-modal">
          <div className="bg-white rounded-[20px] max-w-2xl w-full p-8">
            ...
            <div className="space-y-8">
              {/* Shipping Details */}
              <div>
                <h3 className="text-[24px] font-semibold text-gray-900 mb-4" data-test-id="shipping-details-title">
                  Shipping Details
                </h3>
              ...
              {/* Order Summary */}
              <div>
                <h3 className="text-[24px] font-semibold text-gray-900 mb-4" data-test-id="order-summary-title">
                  Order Summary
                </h3>
                <div className="space-y-3">
                  {cart.map((item) => (
                    <div key={item.id} className="flex justify-between text-[16px] text-gray-600" data-test-id={`confirmation-item-${item.id}`}>
                      <span>{item.name} x {item.quantity}</span>
                      <span>€{(item.price * item.quantity).toFixed(2)}</span>
                    </div>
                  ))}
                  <div className="flex justify-between font-bold text-[18px] text-gray-900 pt-4 border-t" data-test-id="confirmation-total">
                    <span>Total:</span>
